...
class UsuarioController {
    ...
    public function index($param = null) {
        requireRol('administrador'); // supervisor NO ve usuarios
    ...
    public function crear($param = null) {
        ...
        if ($error) {
            $usuario = null; $page_title = 'Nuevo usuario';
            require_once APP_PATH . '/views/shared/layout_admin.php';
            require_once APP_PATH . '/views/admin/usuario_form.php';
            require_once APP_PATH . '/views/shared/layout_admin_footer.php';
            return;
    ...
    public function editar($id = null) {
        ...
        $error = ''; $page_title = 'Editar — ' . $usuario['nombre'];
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!$error) {
                $datos = ['nombre'=>trim($_POST['nombre']),'apellido'=>trim($_POST['apellido']),'email'=>trim($_POST['email']),'rol'=>$_POST['rol']];
                if (!empty($_POST['password'])) $datos['password'] = md5($_POST['password']);
    ...
    public function toggle($id = null) {
        requireRol('administrador');
        if ((int)$id !== (int)$_SESSION['usuario_id']) Usuario::toggleActivo((int)$id);
        header('Location: ' . APP_URL . '/usuarios'); exit;
    }

    public function eliminar($id = null) {
        requireRol('administrador');
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && (int)$id !== (int)$_SESSION['usuario_id']) {
